...
class WebSearchResult(TypedDict):
    """A single web search hit from an Iraqi legal source.

    Produced by the ``web_search`` node via :mod:`src.legal_search`.
    Injected into the translator and auditor prompts as additional context
    to ground the translation in real Iraqi law names and terminology.
    """
...
class TranslationState(TypedDict):
    ...
    - ``web_search`` reads ``input_text``; writes ``web_search_results``.
